...
def loss_piston(k, uR, uI, uR_X, uI_X, uR_XX, uI_XX,
    ...
    loss_int = C0 * tf.reduce_mean(tf.square((uR_XX[:, 0:1] + uR_XX[:, 1:2]) / k**2 + uR - fR)) \
        + C1 * \
        tf.reduce_mean(
            tf.square((uI_XX[:, 0:1] + uI_XX[:, 1:2]) / k**2 + uI - fI))

    loss_b = C2 * tf.reduce_mean(tf.square((uR_X_b[:, 0:1] * X_b[:, 0:1] + uR_X_b[:, 1:2] * X_b[:, 1:2]) / k + uI_b)) \
        + C3 * tf.reduce_mean(tf.square(
            (uI_X_b[:, 0:1] * X_b[:, 0:1] + uI_X_b[:, 1:2] * X_b[:, 1:2]) / k - uR_b))
...
def loss_piston_adapt(k, uR, uI, uR_X, uI_X, uR_XX, uI_XX,
    ...
    loss_b1_r = tf.reduce_mean(tf.square(
        (uR_X_b[:, 0:1] * X_b[:, 0:1] + uR_X_b[:, 1:2] * X_b[:, 1:2]) / k + uI_b))
    loss_b1_i = tf.reduce_mean(tf.square(
        (uI_X_b[:, 0:1] * X_b[:, 0:1] + uI_X_b[:, 1:2] * X_b[:, 1:2]) / k - uR_b))
