...
def get_dipole_solution(x1, x2, k , r0):
  ...
  A_1 = (dH2_dz(1, k) - 1j * H2(1, k)) / (H1(1, k*r0) * (dH2_dz(1, k) - 1j * H2(1, k)) - H2(1, k*r0) * (dH1_dz(1, k) - 1j * H1(1, k)))
  C_1 = (dH1_dz(1, k) - 1j * H1(1, k)) / (H1(1, k*r0) * (dH2_dz(1, k) - 1j * H2(1, k)) - H2(1, k*r0) * (dH1_dz(1, k) - 1j * H1(1, k)))
...
def get_piston_solution(x1, x2, k, r0):
  ...
  for n in range(0, 8):
    ...
    A_n = f_n * (dH2_dz(n, k) - 1j * H2(n, k)) / (H1(n, k*r0) * (dH2_dz(n, k) - 1j * H2(n, k)) - H2(n, k*r0) * (dH1_dz(n, k) - 1j * H1(n, k)))
    C_n = f_n * (dH1_dz(n, k) - 1j * H1(n, k)) / (H1(n, k*r0) * (dH2_dz(n, k) - 1j * H2(n, k)) - H2(n, k*r0) * (dH1_dz(n, k) - 1j * H1(n, k)))
...
def plot_results(PINN_list):
    ...
    for PINN_dict in PINN_list:
        ...
        fig.suptitle('Loss history and solution for k = {} $\pi$'.format(round(k / np.pi)), fontsize=12)
        ...
        real = axs[1, 0].tricontourf(triang, uR, 20, cmap='seismic', levels=100)
        ...
        abs = axs[1, 2].tricontourf(triang, u_abs, 20, cmap='inferno', levels=100)
        ...
        abs_diff = axs[2, 2].tricontourf(triang, np.abs(u_abs - U_abs), 20, cmap='Reds', levels=100)
...
PINN_list = np.load('PINN_list_piston_k=16pi.npy', allow_pickle=True)
